...
def glance_upload_launch(image_filename, creds = {'auth_url': None, 'password': None, 'strategy': 'noauth', 'tenant': None, 'username': None},
    ...
    if (k.authenticate()):
        ...
        cinder = cinder_client.Client('2', creds['username'], creds['password'], 'admin', creds['auth_url'])
        #Connect to nova to start an instance with the volume we created in cinder
        nova = nova_client.Client(creds['username'], creds['password'], 'admin',
...
glance_host = "10.16.17.14"
glance_port = 9292

creds = { 'auth_url': 'http://10.16.17.14:5000/v2.0', 'password': 'eb6721ec1fd593ebebe4', 'strategy': 'keystone', 'tenant': 'admin', 'username': 'admin' }

filename = sys.argv[1]


glance_upload_launch(filename, creds = creds, host = glance_host, port = glance_port, token = None, name = 'Factory Test Image - %s' % (filename), 
                      disk_format = 'qcow2')
